/**
 * \file
 * \brief           k1921vg1t flash driver
 * \copyright       NIIET, 2024
 */
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
#define FLASH_DATA2            (FLASH_BASE + 0x18)
#define FLASH_DATA3            (FLASH_BASE + 0x1C)
...
#define CFGWORD_PAGE                63
#define CFGWORD_ADDR_OFFSET         0x7F0
 ...
 */
static int k1921vg1t_flash_waitdone(struct target *target)
{
	int retval;
	int timeout = 5000;
 ...
 * Erase flash sector
 */
static int k1921vg1t_flash_erase(struct target *target, int page_num, uint32_t region)
{
	int retval = ERROR_OK;
	...
	if (retval != ERROR_OK)
		return retval;
	retval = k1921vg1t_flash_waitdone(target);
	if (retval != ERROR_OK)
		return retval;
 ...
 * Mass flash erase
 */
static int k1921vg1t_flash_mass_erase(struct target *target, uint32_t region)
{
	int retval = ERROR_OK;
 ...
 * Read flash address
 */
static int k1921vg1t_flash_read(struct target *target, uint32_t addr, uint32_t *data, uint32_t region)
{
	int retval = ERROR_OK;
 ...
 * Write flash address
 */
static int k1921vg1t_flash_write(struct target *target, uint32_t addr, uint32_t *data, uint32_t region)
{
	int retval = ERROR_OK;
 ...
 * Dump flash sector.
 */
static int k1921vg1t_flash_sector_dump(struct target *target, uint32_t *dump, int page_num, uint32_t region)
{
	int retval = ERROR_OK;
	...
	for (int i = first; i < last; i+=FLASH_WORD_WIDTH*4) {
		retval = k1921vg1t_flash_read(target, i, data, region);
		if (retval != ERROR_OK)
			return retval;
 ...
 */
static int k1921vg1t_flash_sector_load(struct target *target, uint32_t *dump, int page_num, uint32_t region)
{
	int i;
	int retval = ERROR_OK;
	...
	retval = k1921vg1t_flash_erase(target, page_num, region);
	if (retval != ERROR_OK)
		return retval;
	...
	for (i = first; i < last; i+=FLASH_WORD_WIDTH*4) {
		...
		retval = k1921vg1t_flash_write(target, i, data, region);
		if (retval != ERROR_OK)
			return retval;
	}
 ...
 * Read CFGWORD
 */
static int k1921vg1t_flash_read_cfgword(struct target *target, uint32_t *cfgword)
{
	int retval = ERROR_OK;

	uint32_t data[FLASH_WORD_WIDTH];

	retval = k1921vg1t_flash_read(target, CFGWORD_ADDR, data, NVR_REGION);
	if (retval != ERROR_OK)
		return retval;
 ...
 * Modify CFGWORD
 */
static int k1921vg1t_flash_modify_cfgword(struct target *target, uint32_t enable, uint32_t param_mask)
{
	int retval = ERROR_OK;

	/* dump */
	uint32_t flash_dump[FLASH_PAGE_SIZE/4];
	retval = k1921vg1t_flash_sector_dump(target, flash_dump, CFGWORD_PAGE, NVR_REGION);
	if (retval != ERROR_OK)
		return retval;

	/* modify dump */
	if (enable) /* we need to clear bit to enable */
		flash_dump[(CFGWORD_ADDR_OFFSET>>2)] &= ~param_mask;
	else
		flash_dump[(CFGWORD_ADDR_OFFSET>>2)] |= param_mask;

	/* write dump to flash */
	retval = k1921vg1t_flash_sector_load(target, flash_dump, CFGWORD_PAGE, NVR_REGION);
	if (retval != ERROR_OK)
		return retval;
 ...
 *==============================================================================
 */
COMMAND_HANDLER(k1921vg1t_handle_read_command)
{
	if (CMD_ARGC < 2)
	...
	retval = k1921vg1t_flash_read(target, flash_addr, flash_data, region);
	if (retval != ERROR_OK)
		return retval;
...
COMMAND_HANDLER(k1921vg1t_handle_write_command)
{
	if (CMD_ARGC < 5)
		return ERROR_COMMAND_SYNTAX_ERROR;
	...
	if (save_sector) {
		...
		retval = k1921vg1t_flash_sector_dump(target, flash_dump, page_num, region);
		if (retval != ERROR_OK)
			return retval;
		...
		retval = k1921vg1t_flash_sector_load(target, flash_dump, page_num, region);
		if (retval != ERROR_OK)
			return retval;
	} else {
		if (erase_sector) {
		...
		retval = k1921vg1t_flash_write(target, flash_addr, flash_data, region);
		if (retval != ERROR_OK)
			return retval;
	}
...
COMMAND_HANDLER(k1921vg1t_handle_mass_erase_command)
{
	if (CMD_ARGC < 1)
		return ERROR_COMMAND_SYNTAX_ERROR;
	...
	retval = k1921vg1t_flash_mass_erase(target, region);
	if (retval != ERROR_OK)
		return retval;
...
COMMAND_HANDLER(k1921vg1t_handle_erase_command)
{
	if (CMD_ARGC < 3)
		return ERROR_COMMAND_SYNTAX_ERROR;
	...
	for (unsigned int i = first; i <= last; i++) {
		retval = k1921vg1t_flash_erase(target, i, region);
		if (retval != ERROR_OK)
			return retval;
	}
...
COMMAND_HANDLER(k1921vg1t_handle_protect_command)
{
	if (CMD_ARGC < 2)
		return ERROR_COMMAND_SYNTAX_ERROR;
	...
	retval = k1921vg1t_flash_modify_cfgword(target, protect_enable, param_mask);
	if (retval != ERROR_OK)
		return retval;
...
COMMAND_HANDLER(k1921vg1t_handle_jtag_command)
{
	if (CMD_ARGC < 1)
		return ERROR_COMMAND_SYNTAX_ERROR;
	...
	retval = k1921vg1t_flash_modify_cfgword(target, jtag_enable, param_mask);
	if (retval != ERROR_OK)
		return retval;
...
COMMAND_HANDLER(k1921vg1t_handle_srv_erase_command)
{
	int retval;
	struct target *target = get_current_target(CMD_CTX);
...
COMMAND_HANDLER(k1921vg1t_handle_driver_info_command)
{
	int retval = ERROR_OK;

	command_print(CMD, "k1921vg1t flash driver\n"
						   "version: %d.%d\n"
						   "copyright: AO NIIET, 2024\n",
						   FLASH_DRIVER_VER>>16,
...
static const struct command_registration k1921vg1t_exec_command_handlers[] = {
	{
		.name = "read",
		.handler = k1921vg1t_handle_read_command,
		.mode = COMMAND_EXEC,
	...
	{
		.name = "write",
		.handler = k1921vg1t_handle_write_command,
		.mode = COMMAND_EXEC,
	...
	{
		.name = "mass_erase",
		.handler = k1921vg1t_handle_mass_erase_command,
		.mode = COMMAND_EXEC,
		.usage = "(main|nvr)",
	...
	{
		.name = "erase",
		.handler = k1921vg1t_handle_erase_command,
		.mode = COMMAND_EXEC,
	...
	{
		.name = "protect",
		.handler = k1921vg1t_handle_protect_command,
		.mode = COMMAND_EXEC,
	...
	{
		.name = "jtag",
		.handler = k1921vg1t_handle_jtag_command,
		.mode = COMMAND_EXEC,
	...
	{
		.name = "srv_erase",
		.handler = k1921vg1t_handle_srv_erase_command,
		.mode = COMMAND_EXEC,
		.usage = "",
	...
	{
		.name = "driver_info",
		.handler = k1921vg1t_handle_driver_info_command,
		.mode = COMMAND_EXEC,
		.usage = "",
...
static const struct command_registration k1921vg1t_command_handlers[] = {
	{
		.name = "k1921vg1t",
		.mode = COMMAND_ANY,
		.help = "k1921vg1t flash command group",
		.usage = "",
		.chain = k1921vg1t_exec_command_handlers,
	},
	COMMAND_REGISTRATION_DONE
};
...
FLASH_BANK_COMMAND_HANDLER(k1921vg1t_flash_bank_command)
{
	struct k1921vg1t_flash_bank *k1921vg1t_info;

	if (CMD_ARGC < 6)
		return ERROR_COMMAND_SYNTAX_ERROR;

	k1921vg1t_info = malloc(sizeof(struct k1921vg1t_flash_bank));

	bank->driver_priv = k1921vg1t_info;

	/* information will be updated by probing */
	k1921vg1t_info->probed = false;
	k1921vg1t_info->chip_name = "k1921vg1t";
	k1921vg1t_info->flashwe = true;
	k1921vg1t_info->cfgwe = true;

	return ERROR_OK;
}

static int k1921vg1t_protect_check(struct flash_bank *bank)
{
	struct k1921vg1t_flash_bank *k1921vg1t_info = bank->driver_priv;

	uint32_t protect_enable;

	protect_enable = !k1921vg1t_info->flashwe ? 1 : 0;
...
static int k1921vg1t_mass_erase(struct flash_bank *bank)
{
	struct target *target = bank->target;
	int retval;

	retval = k1921vg1t_flash_mass_erase(target, MAIN_REGION);
	if (retval != ERROR_OK)
		return retval;
...
static int k1921vg1t_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	struct target *target = bank->target;
	...
	if ((first == 0) && (last == (bank->num_sectors - 1))) {
		retval = k1921vg1t_mass_erase(bank);
		if (retval != ERROR_OK)
			return retval;
	} else {
		/* erasing pages */
		for (unsigned int i = first; i <= last; i++) {
			retval = k1921vg1t_flash_erase(target, i, MAIN_REGION);
			if (retval != ERROR_OK)
				return retval;
			bank->sectors[i].is_erased = 1;
		}
...
static int k1921vg1t_protect(struct flash_bank *bank, int set, unsigned int first, unsigned int last)
{
	struct target *target = bank->target;
	struct k1921vg1t_flash_bank *k1921vg1t_info = bank->driver_priv;
	...
	retval = k1921vg1t_flash_modify_cfgword(target, set, param_mask);
	if (retval != ERROR_OK)
		return retval;
	k1921vg1t_info->flashwe = false;
	return retval;
}

static int k1921vg1t_write_block(struct flash_bank *bank, const uint8_t *buffer,
		uint32_t offset, uint32_t count)
{
	...
	static const uint8_t k1921vg1t_flash_write_code[] = {
#include "../../../contrib/loaders/flash/niiet/k1921vg1t/k1921vg1t.inc"
	};

	LOG_INFO("Start block write");
	/* flash write code */
	if (target_alloc_working_area(target, sizeof(k1921vg1t_flash_write_code),
	...
	retval = target_write_buffer(target, write_algorithm->address,
			sizeof(k1921vg1t_flash_write_code),k1921vg1t_flash_write_code);
	if (retval != ERROR_OK){
...
static int k1921vg1t_write(struct flash_bank *bank, const uint8_t *buffer,
		uint32_t offset, uint32_t count)
{
	struct target *target = bank->target;
	...
	retval = k1921vg1t_write_block(bank, buffer, offset, count/(FLASH_WORD_WIDTH*4));
...
static int k1921vg1t_probe(struct flash_bank *bank)
{
	struct k1921vg1t_flash_bank *k1921vg1t_info = bank->driver_priv;
	struct target *target = bank->target;
	...
	if ((retval != ERROR_OK) || (chipid != SIU_CHIPID_K1921VG1T)) {
		LOG_INFO("CHIPID error %d, CHIPID=%x", retval,chipid);
		return ERROR_FAIL;
	}

	LOG_INFO("k1921vg1t detected");
	...
	if (!service_mode) {
		...
		/* read CFGWORD */
		retval = k1921vg1t_flash_read_cfgword(target, &cfgword);
		if (retval != ERROR_OK)
			return retval;


		k1921vg1t_info->flashwe = (cfgword & CFGWORD_FLASHWE) == CFGWORD_FLASHWE;
		k1921vg1t_info->cfgwe = (cfgword & CFGWORD_CFGWE) == CFGWORD_CFGWE;



		snprintf(k1921vg1t_info->chip_brief,
				sizeof(k1921vg1t_info->chip_brief),
				"\n"
				...
				"    %s\n",
				bank->base,
				k1921vg1t_info->flashwe ? "disable" : "enable",
				k1921vg1t_info->cfgwe ? "disable" : "enable");
	} else {
		sprintf(k1921vg1t_info->chip_brief,
				"\n"
	...
	k1921vg1t_info->probed = true;

	return ERROR_OK;
}

static int k1921vg1t_auto_probe(struct flash_bank *bank)
{
	struct k1921vg1t_flash_bank *k1921vg1t_info = bank->driver_priv;
	if (k1921vg1t_info->probed)
		return ERROR_OK;
	return k1921vg1t_probe(bank);
}

static int get_k1921vg1t_info(struct flash_bank *bank, struct command_invocation *cmd)
{
	struct k1921vg1t_flash_bank *k1921vg1t_info = bank->driver_priv;
	command_print_sameline(cmd, "\nNIIET %s\n%s", k1921vg1t_info->chip_name, k1921vg1t_info->chip_brief);

	return ERROR_OK;
}


const struct flash_driver k1921vg1t_flash = {
	.name = "k1921vg1t",
	.usage = "flash bank <name> k1921vg1t <base> <size> 0 0 <target#>",
	.commands = k1921vg1t_command_handlers,
	.flash_bank_command = k1921vg1t_flash_bank_command,
	.erase = k1921vg1t_erase,
	.protect = k1921vg1t_protect,
	.write = k1921vg1t_write,
	.read = default_flash_read,
	.probe = k1921vg1t_probe,
	.auto_probe = k1921vg1t_auto_probe,
	.erase_check = default_flash_blank_check,
	.protect_check = k1921vg1t_protect_check,
	.info = get_k1921vg1t_info,
